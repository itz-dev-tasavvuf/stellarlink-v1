...
// import { useUserData } from '../../context/UserDataContext'; // Remove or comment this out

interface UserLocationData {
  _id: string; // Assuming your user ID is _id
  name: string;
  title: string;
  profileImage?: string;
  location: {
  ...
  bio?: string; // Add bio if you have it
  interests: string[];
  dreams: any[]; // Define a proper type for dreams
  achievements: any[]; // Define a proper type for achievements
}

interface GlobeVisualizationProps {
  userData: UserLocationData[]; // Add prop for user data
}

// Accept userData prop
const GlobeVisualization: React.FC<GlobeVisualizationProps> = ({ userData }) => {
  const globeRef = React.useRef<any>();
  const navigate = useNavigate();
  // const { users } = useUserData(); // Remove or comment this out
  const [globeControls, setGlobeControls] = React.useState<any>(null);
  const [selectedUser, setSelectedUser] = useState<any>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Prepare marker data for react-globe.gl using userData prop
  ...
  const handleMarkerClick = (userId: string) => {
    const user = userData.find(u => u._id === userId); // Use userData
    if (user) {
